...
export default function Carousel({ instanceId, data, data_length, title }) {
  ...
  function Arrows({ arrowPrevRef, arrowNextRef }) {
    return (
      ...
      >
        <div
          ref={arrowPrevRef}
          className="bg-white text-black dark:bg-black p-2 splide__arrow splide__arrow--prev transition-opacity duration-600 ease-in border border-black hover:text-neutral-500"
        >
          <IoMdArrowDropleft
            size={30}
          />
        ...
        <div
          ref={arrowNextRef}
          className="bg-white text-black dark:bg-black border border-black p-2 splide__arrow splide__arrow--next transition-opacity duration-600 ease-in hover:text-neutral-500"
        >
          <IoMdArrowDropright
            size={30}
          />
  ...
  // function handleClickSligingModal(id) {
  //   handleFetchSlidingModal(id)
  //   const el = slidingModalRef.current
  //   if (el.style.transform.includes('translateX(500px)')) {
  //     setTimeout(() => {
  //       isOpenSlidingModal.current = true
  //       el.style.transition = 'transform 0.8s cubic-bezier(0.42, 0, 0.58, 1)'
  //       el.style.transform = `translateX(0px)`
  //     }, 300)
  //   } else if (el.style.transform.includes('translateX(0px)')) {
  //     setTimeout(() => {
  //       isOpenSlidingModal.current = false
  //       el.style.transition = 'transform 0.8s cubic-bezier(0.42, 0, 0.58, 1)'
  ...
  function SlidingModal() {
    ...
    return (
      <div
        ...
        className="fixed w-[500px] h-screen bg-black right-0 top-0 z-50 overflow-y-auto"
      >
        <div
            ...
            >
                {
                  [0, 0, 0, 0, 0, 0, 0, 0, 0].map((i, index) => (
                    ...
                    >
                      ...
                      >
                        <div
                          className="w-full h-[450px] relative shrink-0"
                        >
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {
                [...data, ...data, ...data, ...data, ...data, ...data, ...data, ...data, ...data, ...data, ...data,].map((i, index) => (
                  <SplideSlide
                    className='relative'
                    key={index}
                  >
                    <div
                      ...
                      className={`group w-[250px]¿ w-full gap-y-2 flex flex-col hover:scale-105 transition-transform duration-300 ease-in slide__item__sliding__modal#id${instanceId}`}
                    >
                      <div
